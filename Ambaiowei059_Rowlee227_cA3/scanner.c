...
Token aa_func02(char lexeme[]){
	...
	int length;
	int vid_offset = 0;
	int index = iskeyword(lexeme); /*if the lexeme is a keyword.*/

	if (index >= 0) { 
		t.code = KW_T;
		t.attribute.kwt_idx = index; /* Set the KW attribute to the keyword index in the keyword table array. */
		return t;
	}
	...
	/*If a variable begins with letters i,o,d,w the variable is of type integer(PLATYPUS informal language specification)*/
	if (lexeme[0] == 'i' || lexeme[0] == 'o' || lexeme[0] == 'd' || lexeme[0] == 'w'){

		vid_offset = st_install(sym_table, lexeme, 'I', line);
	}
	else{
		vid_offset = st_install(sym_table, lexeme, 'F', line);
	}

	if (length > VID_LEN){ /* If the variable name ends with (%), the variable is considered of type string*/
		lexeme[VID_LEN] = '\0';
	}

	/*If the symbol table is full,call the st_store() function and terminate the program with a call to exit()*/
	...
	/*sets the token attribute vid_offset to the offset returned by the function*/
...
Token aa_func03(char lexeme[]){
	...
	/* if the size of the lexeme is out of range*/
	if (length > VID_LEN){
		lexeme[VID_LEN - 1] = '%';
		lexeme[VID_LEN] = '\0';
	}

	/* If the variable name ends with (%), the variable is considered of type string*/
	vid_offset = st_install(sym_table, lexeme, 'S', line);

	/*If the symbol table is full,must call st_store() and terminate the program with a call to exit()*/
	if (sym_table.st_offset == sym_table.st_size){
		printf("\nError: The Symbol Table is full - install failed.\n");
		st_store(sym_table);
		exit(EXIT);
...
long atool(char * lexeme){
	...
	/*int octalValue = atoi(lexeme);
	int powerOfNumber = 0;
	int remainder = 0;
	long intValueOfOctal = 0;

	while (octalValue != 0){
	remainder = octalValue % 10;
	...
	intValueOfOctal = strtol(lexeme, NULL, 8);
